        ...
        if "@\\".chars().any(|c| c == tok) {
            ...
            match tag.as_str() {
                ...
                "returns" | "return" | "result" => {
                    emit_section_header(&mut res, "# Returns");
                    // List item (like @param) so multiple returns don't run together.
                    res.push("* ".to_owned());
                }
            ...
            // Collapse indentation, but keep a blank line as a paragraph break.
            let mut blank = false;
            toks.skip_while(|&c| {
                if c == '\n' {
                    blank = true;
...
mod tests {
    ...
    // issue #3
    #[test]
    fn multiple_returns_are_listed() {
        const S: &str = "\\return DPFPDD_SUCCESS: Library was initialized;\n\\return DPFPDD_E_FAILURE: Failed to initialize library.";
        const S_: &str = "# Returns\n\n* DPFPDD_SUCCESS: Library was initialized;\n* DPFPDD_E_FAILURE: Failed to initialize library.";
        assert_eq!(crate::transform(S).unwrap(), S_);
    }

    // issue #3
    #[test]
    fn blank_line_paragraph_break_preserved() {
        const S: &str = "First paragraph.\n\nSecond paragraph.";
        const S_: &str = "First paragraph.\n\nSecond paragraph.";
        assert_eq!(crate::transform(S).unwrap(), S_);
    }
